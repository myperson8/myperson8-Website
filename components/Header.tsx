  ...
  return (
    <header className="sticky top-0 z-50 glass-card border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
      <div className="container mx-auto px-4 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
        <Link to="/home" className="text-2xl font-black hover:opacity-80 transition flex items-center gap-3">
          <img 
        ...
        <nav className="flex flex-wrap justify-center items-center gap-1 sm:gap-2">
          ...
          {user ? (
            <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-800/50 p-1.5 pl-3 rounded-2xl border border-slate-200 dark:border-slate-700 group relative">
              <span className="text-xs font-bold text-slate-600 dark:text-slate-300 truncate max-w-[100px]">{user.username}</span>
              <img src={user.avatar} alt={user.username} className="w-8 h-8 rounded-xl shadow-md border border-white dark:border-slate-600" />
              
              {/* Quick Logout Tooltip */}
              <button 
                onClick={logout}
                className="absolute -bottom-10 left-1/2 -translate-x-1/2 px-3 py-1 bg-red-600 text-white text-[10px] font-bold rounded-lg opacity-0 group-hover:opacity-100 transition pointer-events-none group-hover:pointer-events-auto whitespace-nowrap"
              >
                Logout
              </button>
            </div>
            ...
            <Link 
              to="/login"
              className="flex items-center gap-2 px-4 py-2 bg-[#5865F2] hover:bg-[#4752C4] text-white text-xs font-black rounded-xl transition shadow-lg active:scale-95"
            >
          ...
          <button
            onClick={toggleTheme}
            className="ml-2 p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
